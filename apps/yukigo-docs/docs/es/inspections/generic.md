...
| `Declares`                        | ¿se declara el elemento especificado?
| `DeclaresComputation`             | ¿existe el cómputo especificado (método, predicado, función, etc.)?
| `DeclaresComputationWithArity`    | ¿el cómputo especificado no toma argumentos?
...
| `DeclaresTypeAlias`               | ¿se declara un sinónimo/alias de tipo especificado?
| `DeclaresTypeSignature`           | ¿se declara la firma de tipo de un cómputo especificado?
| `DeclaresVariable`                | ¿se declara una variable local o global especificada?
| `Delegates`                       | ¿se declara *y* llama a un método, función o procedimiento no vacío?
...
| `Uses`                            | ¿existe alguna referencia al elemento especificado?
| `UsesArithmetic`                  | ¿se utilizan operadores aritméticos?
| `UsesConditional`                 | ¿se utiliza alguna estructura de control condicional?
| `UsesExceptionHandling`           | ¿se maneja alguna _excepción_?
| `UsesExceptions`                  | ¿se lanza alguna _excepción_?
| `UsesFor`                         | ¿se utiliza algún tipo de comprensión o repetición indexada?
| `UsesIf`                          | ¿se utiliza una estructura de control `if`?
| `UsesLogic`                       | ¿se utilizan operadores booleanos / lógicos?
| `UsesMath`                        | ¿se utilizan operadores aritméticos?
...
| `HasRedundantBooleanComparison`   | ¿se realiza una comparación booleana redundante?
...
| `HasWrongCaseIdentifiers`         | ¿un identificador no coincide con el estilo de convención de nombres (case style) del lenguaje de dominio?
