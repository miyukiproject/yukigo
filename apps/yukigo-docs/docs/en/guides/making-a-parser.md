...
Every parser needs to expose a `parse` and `parseExpression` methods, and can also include an errors array. The internal checks or functionality isn't important to Yukigo. For example, if the language is typed, we could add a type checker step before returning the parsed AST.
...
> You can check the Yukigo's AST reference [here](/en/ast/README) 
...
This is not hard to implement. We need to define a primary rule `list` and use `ListPrimitive` node from `yukigo-ast`. Let's add it
